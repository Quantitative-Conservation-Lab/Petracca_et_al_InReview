...
setwd("G:/My Drive/GitHub/Wolves")

#source R code with function

#turn on correct multinomial function
load("Outputs/spatial_model/territory_choice.RData") #RSF
#load("Outputs/spatial_model/territory_choice_LCP_multinom.RData") #LCP

#turn on correct movement function
source("Scripts/a_composite_model/movement_function.R")
# source("Scripts/a_composite_model/movement_function_singlelcp_5thpercent.R")
# source("Scripts/a_composite_model/movement_function_MNlcp_5thpercent.R")
# source("Scripts/a_composite_model/movement_function_singlelcp_RSFmultinom.R")
# source("Scripts/a_composite_model/movement_function_MNlcp_RSFmultinom.R")
# source("Scripts/a_composite_model/movement_function_singlelcp_staycollar.R")
# source("Scripts/a_composite_model/movement_function_MNlcp_staycollar.R")

source("Scripts/a_composite_model/removals_function.R")
source("Scripts/a_composite_model/translocation_function.R")
source("Scripts/a_composite_model/attraction_function.R")
...
load("Outputs/a_composite_model/Projection_Inputs.RData")

#read in files relating to movement function
# load("Outputs/spatial_model/prob_stay_multinom.RData")
# load("Outputs/spatial_model/terr_greater_5thpercent.RData")
...
additive <- 1 #fully additive (0.5 means partly compensatory)
...
Nglobal_state.mean <- Nglobal_state_wmove.mean <- NAdult_state.mean <- #NAdult_state_wsolo.mean <- 
...
for(sim in 1:nSims){
  ...
  ###### THIS IS FOR SECOND PERIOD OF FIRST YEAR ONLY #####
  ...
  if(analysis=="disease"){
    ...
    phiA.proj[,,time:(time+1)] <- phiA.proj[,,time:(time+1)] * (1-(disease_prop*additive)) 
    phiB.proj[,,time:(time+1)] <- phiB.proj[,,time:(time+1)] * (1-(disease_prop*additive)) 
  }
  ...
  for (s in 1:S){ #running through all 224 territories
    ...
    N.proj[,1,2,1,s] <- rbinom(nSamples, N.proj[,1,1,1,s], phiA.proj[,1,1])
  ...
  n.res <- N.proj[,1,2,1,] + N.stayers.proj[,2,2,1,] + N.stayers.proj[,3,2,1,]
  
  # n.immig <- array(NA,dim = c(nSamples,224))
  # n.immig <- N.immig.proj[,2,2,1,] + N.immig.proj[,3,2,1,]
  ...
  for(i in 1:nSamples){ #nSamples samples
    ...
    immig_id <- which(N.proj[i,1,2,1,] + N.stayers.proj[i,2,2,1,] + N.stayers.proj[i,3,2,1,]+
                        new.guys[[1]][i,] + new.guys[[2]][i,] >0)
    #immig_id <- intersect(immig_id,immig_terr_id)
  ...
  for (t in 1:(proj-1)){ #this is the big outer t loop
    ...
    for (s in 1:S){ #running through all 224 territories
      ...
      N.movers.proj[,2,1,t+1,s] <- rbinom(nSamples, N.proj[,1,2,t,s], phiA.proj[,2,2*t]*epsA[,1]*alpha) #N.MOVERS IS NOW SPATIAL 
    ...
    n.settlers.for.fxn <- array(NA,dim = c(nSamples,2,224))
    n.settlers.for.fxn <- N.settlers.proj[,c(2:3),1,t+1,] #just getting nSamples x 2 x site
    
    n.res <- array(NA,dim = c(nSamples,224))
    n.res <- N.proj[,1,1,t+1,] + N.stayers.proj[,2,1,t+1,] + N.stayers.proj[,3,1,t+1,]
    
    # n.immig <- array(NA,dim = c(nSamples,224))
    # n.immig <- N.immig.proj[,2,1,t+1,] + N.immig.proj[,3,1,t+1,]
    ...
    for (s in 1:S){ #start s loop again
      ...
      N.proj[,1,2,t+1,s] <- rbinom(nSamples, N.proj[,1,1,t+1,s], phiA.proj[,1,((2*t)+1)])
    ...
    n.res <- array(NA,dim = c(nSamples,224))
    n.res <- N.proj[,1,2,t+1,] + N.stayers.proj[,2,2,t+1,] + N.stayers.proj[,3,2,t+1,]
    
    # n.immig <- array(NA,dim = c(nSamples,224))
    # n.immig <- N.immig.proj[,2,2,t+1,] + N.immig.proj[,3,2,t+1,]
    
    #call function - you will repeat this 3 times in the code 
    # put this source at the beginning of code, not in loop: source(function_try.R)
    new.guys <- get.move(n.settlers.for.fxn,n.res,site_check,array_probs,array_siteID)  
    ...
    for(i in 1:nSamples){
      #which ids are occupied?
      immig_id <- which(N.proj[i,1,2,t+1,] + N.stayers.proj[i,2,2,t+1,] + N.stayers.proj[i,3,2,t+1,]+
                          new.guys[[1]][i,] + new.guys[[2]][i,] >0)
      #immig_id <- intersect(immig_id,immig_terr_id)
  ...
  for(i in 1:nSamples){
    for(t in 1:proj){
      ...
      # Nglobal_EWash.proj[i,t] <- sum(Ntot.proj[i,t,EWash])
      # Nglobal_NCasc.proj[i,t] <- sum(Ntot.proj[i,t,NorthCasc])
      # Nglobal_SCasc.proj[i,t] <- sum(Ntot.proj[i,t,SouthCasc])
      NAdult_state.proj[i,t] <- sum(N.proj[i,2,1,t,]) + sum(N.proj[i,3,1,t,])
      #NAdult_state.wsolo.proj[i,t] <- sum(N.proj[i,2,1,t,]) + sum(N.proj[i,3,1,t,])
  ...
  for (t in 2:proj) {
    ...
    #lambda.proj[t-1] <- mean(lambda.proj.intermed[,t-1], na.rm = T) 
    #lambda.proj.lower[t-1] <-  quantile(lambda.proj.intermed[,t-1], probs = 0.025, na.rm = T, names = F) 
    #lambda.proj.upper[t-1] <-  quantile(lambda.proj.intermed[,t-1], probs = 0.975, na.rm = T, names = F) 
  } #closes t on lambda
  ...
  #lambda.mean[sim] <- exp(mean(log(lambda.proj[which(lambda.proj>0)])))
  #lambda.lower[sim] <- exp(mean(log(lambda.proj.lower[which(lambda.proj.lower>0)])))
  #lambda.upper[sim] <- exp(mean(log(lambda.proj.upper[which(lambda.proj.upper>0)])))
  
  # #prob of quasi-extinction at any point in 100 years
  ...
  #NAdult_state_wsolo.mean[1:nSamples,,sim] <- as.matrix(NAdult_state.wsolo.proj)
...
save(Lambda.mean, #lambda.upper, lambda.lower,
     #p.quasiext, p.recovery,
     Ntot.site_mean, Ntot.site_median, Newguys.mean,
     BP_Presence_summary, Pack_Size_max,Two_Adult_summary,
     Nglobal_state.mean, Nglobal_state_wmove.mean,
     NAdult_state.mean, #NAdult_state_wsolo.mean,
     NAdult_EWash.mean, NAdult_NCasc.mean, NAdult_SCasc.mean, Newguys.mean,
     NSite_state.mean, NSite_EWash.mean, NSite_NCasc.mean, NSite_SCasc.mean, file="Outputs/a_composite_model/Scenarios/6_disease_50perc_additive_RSF_Oct31.RData")
