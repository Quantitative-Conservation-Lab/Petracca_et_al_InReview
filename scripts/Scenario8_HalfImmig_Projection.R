...
for(sim in 1:nSims){
  ...
  for(i in 1:nSamples){ #loop over nSamples to see which territories are occupied 
    ...
    lambda.immig.t <- lambda.immig.mat[1,i]*(assmp.immig/length(immig_id))
  ...
  for (t in 1:(proj-1)){ #this is the big outer t loop
    ...
    for(i in 1:nSamples){
      ...
      #keeps total number of immigrants entering each year to the asymptote from the growth model (ie, limits immigration so it doesn't grow with increasing # of packs)
      lambda.immig.t <- lambda.immig.mat[t+1,i]*(assmp.immig/length(immig_id))
...
save(Lambda.mean,
     ...
     NSite_state.mean, NSite_EWash.mean, NSite_NCasc.mean, NSite_SCasc.mean, file="8_HalfImmig_Projection.RData")