get.translocations <- function(n.wolves.transl.fxn, EWash, site) {
  #in this function, we are removing wolves by pack in E Washington and translocating them to one of two sites -- Mt Ht Helens or the Olympic Peninsula 
  
  dim(n.wolves.transl.fxn)
  
  if(site=="St Helens"){
    packs_translocate <- c(14,10) #these are the packs that qualify based on the RSF & correspond to territory IDs
  }
  if(site=="Olympic"){
  ...
  for(i in 1:Nsamples){    
    ...
    ad.translocated <- rep(0,224)
      ...
      #THIS IS WHERE TRANSLOCATION HAPPENS
          ...
          if(length(packs_qualify)<=1){
            ...
            pup.translocated.all[i,] <- pup.translocated 
